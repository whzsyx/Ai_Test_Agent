...
### 9.4 对象存储（RustFS）与上传安全

| 变量 | 默认 | 说明 |
|------|------|------|
| `ARTIFACT_STORAGE_BACKEND` | rustfs | 工件存储后端 |
| `ARTIFACT_KEEP_LOCAL_COPY` | false | 是否保留本地副本 |
| `RUSTFS_ENDPOINT/ACCESS_KEY/SECRET_KEY/BUCKET/SECURE` | 127.0.0.1:9000 / qa-agent | RustFS 连接 |
| `RUSTFS_UPLOAD_TEMP/SAFE/QUARANTINE_BUCKET` | upload-temp/safe/quarantine | 上传三段式安全桶 |
